...
void setup() {
  ...
  lcd.setCursor(2,1);
  lcd.print("Een moment a.u.b.");
  lcd.setCursor(3,2);
  lcd.print("de verbinding");
  lcd.setCursor(3,3);
  lcd.print("wordt getest");
  
  // Give time to your GSM shield log on to network
  delay(10000);
  lcd.begin(20, 4);
  lcd.println(F("FONA SMS caller ID test"));
  delay(1000);
  lcd.begin(20, 4);
  lcd.println(F("Initializing....(Kan 3 seconden duren)"));
  ...
  if (! fona.begin(*fonaSerial)) {
    lcd.println(F("Couldn't find FONA"));
    while(1);
  }
  lcd.println(F("verbinding is OK!"));

  // Print SIM card IMEI number.
  char imei[16] = {0}; // MUST use a 16 character buffer for IMEI!
  uint8_t imeiLen = fona.getIMEI(imei);
  if (imeiLen > 0) {
    lcd.print("SIM card IMEI: "); lcd.println(imei);}

  lcd.begin(20, 4);  
  lcd.println("Verbinding is OK!");
  delay(1000);
  ...
  // AT command to set to SMS mode
  fona.print("AT+CMGF=1\r"); 
  // Set module to send SMS data to serial out upon receipt 
  fona.print("AT+CNMI=2,2,0,0,0\r");
  delay(100); 
